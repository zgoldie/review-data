import { pgQuery } from '../../api/_lib/db.js'
import { recomputeDurationsForVersion } from '../../api/_lib/durations.js'
...
async function main() {
  ...
  await pgQuery('BEGIN')
  try {
    await pgQuery('TRUNCATE TABLE version_durations, raw_events, summary_bucket_stats, summary_trends RESTART IDENTITY')
  ...
  const pairs = await pgQuery('SELECT DISTINCT user_id, app_version_id FROM raw_events')
  for (const pair of pairs.rows) {
    await recomputeDurationsForVersion(pair.user_id, pair.app_version_id)
  }

  console.log(`Seeded ${events.length} events across ${pairs.rows.length} app versions.`)
}
